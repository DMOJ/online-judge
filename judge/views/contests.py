...
class ContestList(DiggPaginatorMixin, TitleMixin, ContestListMixin, ListView):
    ...
    def _get_queryset(self):
        return (
            ...
            .prefetch_related('tags', 'organizations', 'organizers')
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            for participation in (
                ...
                .prefetch_related('contest__organizers')
            ):
...
class ContestMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            in_contest = context['in_contest'] = (
                profile.current_contest is not None and profile.current_contest.contest == self.object
            )
            if in_contest:
    ...
    def get_object(self, queryset=None):
        ...
        if (
            profile is not None
            and ContestParticipation.objects.filter(id=profile.current_contest_id, contest_id=contest.id).exists()
        ):
            return contest
        ...
        try:
            ...
            raise PrivateContestError(
                contest.name, contest.is_private, contest.is_organization_private, contest.organizations.all()
            )
        ...
        context['contest_problems'] = (
            ...
            .annotate(
                has_public_editorial=Sum(
                    Case(When(solution__is_public=True, then=1), default=0, output_field=IntegerField())
                )
            )
...
class ContestJoin(LoginRequiredMixin, ContestMixin, BaseDetailView):
    ...
    def join_contest(self, request, access_code=None):
        ...
        requires_access_code = (
            not (request.user.is_superuser or self.is_organizer)
        ...
        if contest.ended:
            ...
            while True:
                ...
                try:
                    participation = ContestParticipation.objects.create(
                        contest=contest, user=profile, virtual=virtual_id, real_start=timezone.now()
                    )
            ...
            try:
                ...
                participation = ContestParticipation.objects.create(
                    contest=contest, user=profile, virtual=(-1 if self.is_organizer else 0), real_start=timezone.now()
                )
            else:
                if participation.ended:
                    participation = ContestParticipation.objects.get_or_create(
                        contest=contest, user=profile, virtual=-1, defaults={'real_start': timezone.now()}
                    )[0]
    ...
    def ask_for_access_code(self, form=None):
        ...
        return render(
            ...
            {'form': form, 'wrong_code': wrong_code, 'title': _('Enter access code for "%s"') % contest.name},
        )
...
class ContestLeave(LoginRequiredMixin, ContestMixin, BaseDetailView):
    def post(self, request, *args, **kwargs):
        ...
        if profile.current_contest is None or profile.current_contest.contest_id != contest.id:
            return generic_message(request, _('No such contest'), _('You are not in contest "%s".') % contest.key, 404)
    ...
    def get_context_data(self, **kwargs):
        ...
        stats = {
            'problem_status_count': {
                'labels': labels,
                'datasets': [
                    {'label': name, 'backgroundColor': settings.DMOJ_STATS_SUBMISSION_RESULT_COLORS[name], 'data': data}
...
def contest_ranking_list(contest, problems):
    return base_contest_ranking_list(
        contest,
        problems,
        contest.users.filter(virtual=0, user__is_unlisted=False)
...
def get_contest_ranking_list(
    ...
    if contest.hide_scoreboard and contest.is_in_contest(request.user):
        return (
            [(_('???'), make_contest_ranking_profile(contest, request.profile.current_contest, problems))],
            problems,
        )
...
def contest_ranking_ajax(request, contest, participation=None):
    ...
    return render(
        ...
        {'users': users, 'problems': problems, 'contest': contest, 'has_rating': contest.ratings.exists()},
    )
