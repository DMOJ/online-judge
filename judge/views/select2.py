...
class Select2View(BaseListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {
                'results': [{'text': smart_text(self.get_name(obj)), 'id': obj.pk} for obj in context['object_list']],
...
class ContestUserSearchSelect2View(UserSearchSelect2View):
    def get_queryset(self):
        ...
        if (
            not contest.can_see_scoreboard(self.request.user)
            or contest.hide_scoreboard
            and contest.is_in_contest(self.request.user)
        ):
            raise Http404()
