    ...
    return JsonResponse(
        {
            ...
            'format': {'name': contest.format_name, 'config': contest.format_config},
            'problems': [
                {
...
def api_v1_problem_list(request):
    ...
    return JsonResponse(
        {
            code: {'points': points, 'partial': partial, 'name': name, 'group': group}
...
def api_v1_user_list(request):
    ...
    return JsonResponse(
        {
            username: {'points': points, 'performance_points': performance_points, 'rank': rank}
...
def api_v1_user_info(request, user):
    ...
    submissions = list(
        Submission.objects.filter(
            case_points=F('case_total'), user=profile, problem__is_public=True, problem__is_organization_private=False
        )
        .values('problem')
        .distinct()
    ...
    if not profile.is_unlisted:
        ...
        for contest_key, rating, volatility in participations.values_list(
            'contest__key', 'rating__rating', 'rating__volatility'
        ):
            contest_history[contest_key] = {'rating': rating, 'volatility': volatility}
    ...
    return JsonResponse(
        {
            ...
            for sub in subs.values(
                'id', 'problem__code', 'time', 'memory', 'points', 'language__key', 'status', 'result'
            )
        }
    )
...
def api_v1_user_ratings(request, page):
    ...
    return JsonResponse(
        {'pages': paginator.num_pages, 'users': {username: {'rating': rating} for username, rating in page}}
    )
