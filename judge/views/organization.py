...
class OrganizationUsers(OrganizationDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['users'] = ranker(
            ...
            .order_by('-performance_points', '-problem_count')
            .select_related('user')
...
class RequestJoinOrganization(LoginRequiredMixin, SingleObjectMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            reverse(
                'request_organization_detail', args=(request.organization.id, request.organization.slug, request.id)
            )
        )
        ...
        if formset.is_valid():
            if organization.slots is not None:
                ...
                if to_approve > can_add:
                    messages.error(
                        request,
                        _('Your organization can only receive %d more members. ' 'You cannot approve %d users.')
                        % (can_add, to_approve),
                    )
