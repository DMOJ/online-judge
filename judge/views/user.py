...
class UserPage(TitleMixin, UserMixin, DetailView):
    ...
    def get_title(self):
        return _('My account') if self.request.user == self.object.user else _('User %s') % self.object.display_name
                ...
                [
                    {
                        ...
                        'ranking': rating.rank,
                        'link': '%s#!%s'
                        % (reverse('contest_ranking', args=(rating.contest.key,)), self.object.user.username),
...
class UserPrepareData(LoginRequiredMixin, UserDataMixin, TitleMixin, FormView):
    ...
    def can_prepare_data(self):
        return (
            ...
            or self.request.profile.data_last_downloaded + settings.DMOJ_USER_DATA_DOWNLOAD_RATELIMIT < self._now
    ...
    def in_progress_url(self):
        ...
        return self.build_task_url(status_id) if status in ('PENDING', 'PROGRESS', 'STARTED') else None
    ...
    def get_context_data(self, **kwargs):
        ...
        if not self.can_prepare_data:
            context['time_until_can_prepare'] = settings.DMOJ_USER_DATA_DOWNLOAD_RATELIMIT - (
...
class CustomPasswordResetView(PasswordResetView):
    def post(self, request, *args, **kwargs):
        ...
        if cache.incr(key) > settings.DMOJ_PASSWORD_RESET_LIMIT_COUNT:
            return HttpResponse(
                'You sent in too many password reset requests. Please try again later.',
