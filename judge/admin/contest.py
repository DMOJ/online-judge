...
class ContestAdmin(NoBatchDeleteMixin, VersionAdmin):
    ...
    def set_locked(self, request, queryset):
        ...
        self.message_user(
            request, ungettext('%d contest successfully locked.', '%d contests successfully locked.', count) % count
        )
    ...
    def set_unlocked(self, request, queryset):
        ...
        count = queryset.count()
        self.message_user(
            request, ungettext('%d contest successfully unlocked.', '%d contests successfully unlocked.', count) % count
        )
