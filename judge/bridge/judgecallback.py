...
class DjangoJudgeHandler(JudgeHandler):
    ...
    def on_grading_end(self, packet):
        ...
        for case in SubmissionTestCase.objects.filter(submission=submission).iterator():
            time += case.time
            if not case.batch:
                points += case.points
                total += case.total
    ...
    def on_test_case(self, packet, max_feedback=SubmissionTestCase._meta.get_field('feedback').max_length):
        ...
        for result in updates:
            test_case = SubmissionTestCase(submission_id=id, case=result['position'], kind=result['kind'])
