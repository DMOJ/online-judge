...
class TranslatedProblemQuerySet(SearchQuerySet):
    ...
    def add_i18n_name(self, language):
        ...
        return queryset.annotate(
            i18n_name=Coalesce(RawSQL('%s.name' % alias, ()), F('name'), output_field=models.CharField())
        )
...
class Problem(models.Model):
    ...
    def update_stats(self):
        self.user_count = (
            self.submission_set.filter(points__gte=self.points, result='AC', user__is_unlisted=False)
            .values('user')
            .distinct()
            .count()
        )
        ...
        if submissions:
            self.ac_rate = (
                100.0
                * self.submission_set.filter(points__gte=self.points, result='AC', user__is_unlisted=False).count()
                / submissions
            )
        else:
            self.ac_rate = 0
